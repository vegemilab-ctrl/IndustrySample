...
public class MXRequester : MonoBehaviour
{
    ...
    private MXInterface _MXInterface;
    ...
    {
        _MXInterface.AddGetDeviceRequest(new MXInterface.GetDeviceRequest(deviceAddress, callback));
        _updated = true;
    }
    public void AddSetDeviceRequest(string deviceAddress, short writeValue, Action<bool> callback = null)
    {
        _MXInterface.AddSetDeviceRequest(new MXInterface.SetDeviceRequest(deviceAddress, writeValue, callback));
        _updated = true;
    }
    ...
    {
        _instance = this;
        _MXInterface = new MXInterface(_interval, _capacity, _stationNumber, _password);
    ...
    void Update()
    {
        if (_changed)
        {
            _MXInterface.SetAutoReadDevice(_addressList);
            _changed = false;
        }
